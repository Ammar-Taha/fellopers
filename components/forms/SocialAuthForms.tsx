...
import { SOCIAL_PROVIDERS, type SocialProviderId } from "@/constants/auth";
...
export function SocialAuthForms() {
  return (
      ...
      {SOCIAL_PROVIDERS.map((provider) => (
        <form key={provider.id} action={signInWithOAuth}>
          ...
          <Button
            type="submit"
            variant="outline"
            className="h-10 w-full gap-2 bg-transparent text-xs sm:text-sm dark:bg-dark-400"
          >
